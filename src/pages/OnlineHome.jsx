...
export default function OnlineHome() {
  ...
  return (
    <Box sx={{ maxWidth: 720, mx: 'auto', pb: 4 }}>
      ...
      {selected && (
        <>
          {/* ── ФОКУС ТАЗИ СЕДМИЦА (primary — the main weekly practice) ── */}
          {(visibleTasks.length > 0 || previousHabits.length > 0) && (
            <Paper
              elevation={0}
              data-tour="focus"
              sx={{
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                {visibleTasks.map(task => (
                  ...
                  >
                    ...
                    {task.description && (
                      <InfoOutlinedIcon sx={{ fontSize: 16, color: C.muted, flexShrink: 0 }} />
                    )}
                  </Box>
                ))}
              </Box>

              {/* ── Carry-over: habits from previous weeks ─────────────────
                 Collapsed by default so the focus card stays compact even
                 in week 8. Expand reveals titles grouped by week. */}
              {previousHabits.length > 0 && (
                <Box sx={{ mt: 1.25, pt: 1.25, borderTop: `1px dashed ${C.border}` }}>
                  <Box
    ...
    <Box sx={{ maxWidth: 560, mx: 'auto', pt: { xs: 1, md: 4 }, pb: 4, px: 1 }}>
      ...
      >
        ...
        <Typography sx={{ fontSize: 14, color: C.text, lineHeight: 1.6, mb: 2.5 }}>
          преди да започнеш своята 8-седмична програма, искаме да си наясно
          с няколко неща:
        </Typography>
        ...
        <Box component="ul" sx={{
          ...
          <li>програмата не диагностицира, не лекува и не предписва.</li>
